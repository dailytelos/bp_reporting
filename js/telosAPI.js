...
function hf_itodigits(n) {
	
    var sret = "" + n;

    var adig = sret.substring(0,1);
...
function hf_digitstoi(d) {
	
    var adig = d.substring(0,1);
    var bdig = d.substring(1);
...
function hf_get_xy_from_areaid(area_id) {
	return {
		x: hf_digitstoi(area_id.substr(area_id.length - 4, area_id.length - 2)),
		y: hf_digitstoi(area_id.substr(area_id.length - 2))
	};
}


function hf_getplotindex(x_size, y_size, xpos, ypos) {
	if(xpos >= x_size) { return -1; }
    if(ypos >= y_size) { return -1; }
	
    return xpos + (x_size * ypos);
}
